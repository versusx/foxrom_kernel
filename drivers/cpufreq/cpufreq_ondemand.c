...
extern unsigned int memfreq_increase_cpu_frequency;
...
/* block due to too many log */
#if 0
		printk("****DVFS**** dbs_check_cpu cpu=%u,load=%u\n",j,load);
#endif
	...
	/*increase cpu frequency when mali memory load is high*/
//	printk("****DVFS**** dbs_check_cpu memfreq_increase_cpu_frequency = %d\n",memfreq_increase_cpu_frequency);
	if(memfreq_increase_cpu_frequency)
	{
		dbs_freq_increase(policy, policy->max);
		return;
	}
